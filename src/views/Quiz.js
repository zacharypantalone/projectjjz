  ...
  if (questionCount !== questions.length ) {
    return (
      <div>
        ...
        <button onClick={handleClick}>{questions[questionCount].answer_one}</button>
        <button onClick={handleClick}>{questions[questionCount].answer_two}</button>
      </div>
    )
  } 
  else {
    return (
      <div>
        <button onClick={handleFinalClick}>
          You're all set! Click here to go back to your dashboard and see the results
        </button>
      </div>
    )
  }
...
return (
  <>
    {questions.length > 0 ? <div>{renderQuiz()}</div> : <div>Questions not populated yet</div>}
  </>
  )
  }
